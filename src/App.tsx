...
function App() {
    ...
    switch (col) {
      ...
      case 2:
        if (row < 5 && currArr.includes(tmpField + 8)) {
          ...
          if (currArr.includes(tmpField - 8)) {
            console.log('(1st) diagonalDownRight set && (1st) diagonalTopLeft set')
            if (currArr.includes(tmpField + 16)) {
              console.log('(1st, 2nd) diagonalDownRight set && (1st) diagonalTopLeft set')
              return true
            }
            if (currArr.includes(tmpField - 16)) {
              console.log('(1st) diagonalDownRight set && (1st, 2nd) diagonalTopLeft set && (2nd) diagonalDownRight NOT set')
              return true
            }
          }
          if (currArr.includes(tmpField + 16) && currArr.includes(tmpField + 24)) {
            console.log('(1st, 2nd, 3rd) diagonalTopLeft set && (1st) diagonalDownRight NOT set')
            return true
          }
        }
        return false
  ...
  const setToken = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (gameActive) {
      ...
      if (checkBottom(row, tmpField, currPlayer)) {
        setGameActive(false)
        alert('WON BOTTOM')
      }
      else if (checkSide(col, tmpField, currPlayer)) {
        setGameActive(false)
        alert('WON SIDE')
      }
      else if (checkLeftDiagonal(row, col, tmpField, currPlayer)) {
        setGameActive(false)
        alert('WON LEFT DIAGONAL')
      }
      else if (checkRightDiagonal(row, col, tmpField, currPlayer)) {
        setGameActive(false)
        alert('WON RIGHT DIAGONAL')
      }
      setCount(count + 1)
    }
  }
